...
func (tr *ThisRouter) BuildRoutes(
	...
	if r1 != nil && r2 != nil {
		...
		allResults = append(allResults, RouterResult{
			Route:          unloopingLine(r3), // unloopingLine(unloopingLine(r3))) ?
			RecPointLists:  nil,
			RecRouteFrames: nil,
		})

		allResults = append(allResults, RouterResult{
			Route:          shortByInterval(&r1.Route, &r2.Route),
			RecPointLists:  nil,
			RecRouteFrames: nil,
		})

		allResults = append(allResults, RouterResult{
			Route:          shortByInterval(&r2.Route, &r1.Route),
			RecPointLists:  nil,
			RecRouteFrames: nil,
		})

		allResults = append(allResults, RouterResult{
			Route:          unloopingLine(shortByInterval(&r2.Route, &r1.Route)),
			RecPointLists:  nil,
			RecRouteFrames: nil,
		})
	}
...
// shortByInterval
func shortByInterval(routeA, routeB *Route) Route {
	...
	findFrame := func(point PointOnMap, fl FrameList) (index int, frame *RouteFrame) {
		lenFl := len(fl)
		for index := lenFl - 1; index > 0; index-- {
			if frame := fl[index]; frame.Overlap(point) {
				if frame.IsVerticalLine() {
				} else if frame.IsHorizontalLine() {
				} else {
					// we believe that frame has width=1 or height=1
					panic(frame)
				}
				return index, &frame
			}
	...
	var segments []PointList
	routeAsFramesB := routeB.ToFrames()

	for indexA := from; indexA < to; indexA++ {
		pointA := routeA.Get(indexA)
		indexB, frame := findFrame(pointA, routeAsFramesB)
		...
		if frame != nil {
			...
			if frame[0] != pointA && frame[1] != pointA {
				segments = append(segments, PointList{
					pointA, frame[1],
				})
			}
